...
const OFFLINE_SHELL_CACHE_NAME = "tekstura-offline-shell-v24-app-shell";
...
async function buildOfflineHealthcheckReport() {
  ...
  const offlineStartKeys = [
    "./offline-start.html",
    "/offline-start.html",
    registrationScope ? new URL("./offline-start.html", registrationScope).href : "",
    new URL("/offline-start.html", location.origin).href,
  ].filter(Boolean);
  const foundShellKeys = [];
  const foundOfflineStartKeys = [];
  let cacheNames = [];
  ...
  if ("caches" in window) {
    ...
    for (const url of offlineStartKeys) {
      if (await cache.match(url, { ignoreSearch: true })) {
        foundOfflineStartKeys.push(url);
      }
    }
  }
  ...
  return [
    ...
    `Offline-start cached: ${foundOfflineStartKeys.length ? "да" : "нет"}`,
    `Shell keys: ${foundShellKeys.length ? foundShellKeys.join(", ") : "нет"}`,
    `Offline-start keys: ${foundOfflineStartKeys.length ? foundOfflineStartKeys.join(", ") : "нет"}`,
